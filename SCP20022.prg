...
DEFINE WINDOW record1 FROM 01, 00  TO 24, 79 FLOAT NOCLOSE  ;
       SHADOW TITLE  ;
       'Actualizaciขn de Registro de Inscripciขn (SCP20022)'  ;
...
DO WHILE .T.
     ...
     nod3 = .F.
     IF WVISIBLE('record1')
          ACTIVATE WINDOW SAME record1
     ELSE
          ACTIVATE WINDOW NOSHOW record1
     ENDIF
     IF m.pote = 1
          @ 00, 00 SAY 'ีออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออธ'
          @ 01, 00 SAY 'ณ Cdula  :             Apell/Nombres :                                      ณ'
          @ 02, 00 SAY 'ณ Sexo:   Fecha Nac.:            Lugar Nacmto. :                             ณ'
          @ 03, 00 SAY 'ณ Estado Nacmto:                              Ini./Est.:                     ณ'
          @ 04, 00 SAY 'ณ                                                                            ณ'
          @ 05, 00 SAY 'ฦออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออต'
          @ 06, 00 SAY 'ณ              D A T O S    D E L    R E P R E S E N T A N T E               ณ'
          @ 07, 00 SAY 'รฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤยฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤด'
          @ 08, 00 SAY 'ณ Nombre de la Madre:                          ณ Profesiขn :                 ณ'
          @ 09, 00 SAY 'ณ Nombre  del  Padre:                          ณ Profesiขn :                 ณ'
          @ 10, 00 SAY  ;
     ...
     IF SEEK(xcedula)
          ...
          IF m.saldo > 0
               = msgerro('Este Alumno tiene un registro de deuda vencida pendiente, verifique' )
               IF lapsomat = SPACE(6)
                    IF  .NOT. deuda_ven3()
                         nod1 = .T.
                    ENDIF
     ...
     @ 15, 41 GET m.turno PICTURE  ;
       '@!M MATUTINO,VESPERTINO,NOCTURNO,SABATINO'
     m.codplan = qcurso
     ...
     IF m.turno = 'MAT'
          SELECT 13
          m.control = controlmat
          tno = 'M'
     ENDIF
     IF m.turno = 'VES'
          SELECT 13
          m.control = controlves
          tno = 'V'
     ENDIF
     IF m.turno = 'NOC'
          SELECT 13
          m.control = controlnoc
          tno = 'N'
     ENDIF
     IF m.turno = 'SAB'
          SELECT 13
          m.control = controlsab
          tno = 'S'
     ENDIF
     m.matricula = LTRIM(STR(m.control)) +  ;
                   '-' + tno
     @ 15, 68 SAY m.matricula
     IF LASTKEY() = 27
          p = salida()
          IF p = 2
               opc = acepta( ;
                    ...
                    DO WHILE .T.
                         ...
                         IF  .NOT.  ;
                             FOUND()
                              ...
                              IF qi >  ;
                                 3
                                   = msgerro('Usuario no Autorizado para el acceso al sistema')
                                   CLOSE DATABASES
                                   RELEASE WINDOW
                                   RETURN
                              ENDIF
                             ...
                             10
                              ...
                              IF li >  ;
                                 21
                                   ...
                                   @ 09, 00 SAY 'NUMERO DE CONTROL    : ' + m.matricula
                                   xcantidad = letras(mmontop)
                                   xcantidad = ALLTRIM(xcantidad)
                              ...
                              FOR  ;
                               ...
                               10
                                   ...
                                   ELSE
                                        mds = ALLTRIM(descripcio)
                                   ENDIF
                                   IF mdec11 >= 1 .AND. lin11 = 0
                                        @ li, 00 SAY 'CANCELACION MENSUALIDAD:'
                                        lin11 = 1
                                        li = li + 1
                                   ENDIF
                                   IF mdec11 >= 1 .AND. mdec22 >= 1 .AND. lin22 = 1
                                        @ li, 00 SAY 'ABONO MENSUALIDAD:'
                                        li = li + 1
                                        lin22 = 0
                                   ENDIF
                                   IF mdec11 = 0 .AND. mdec22 >= 1 .AND. lin33 = 0
                                        @ li, 00 SAY 'ABONO MENSUALIDAD:'
                                        li = li + 1
                                        lin33 = 1
                                   ENDIF
                                   ...
                                   ENDIF
                                   @ li, 36 SAY '('
                                   @ li, 38 SAY mmon PICTURE '9,999,999.99'
                                   @ li, 51 SAY ')'
                                   IF ww = 1
                                        @li,58 say mpagrec&lk pict '9,999,999.99'
                                   ENDIF
                                   IF mdec22 >= 1 .AND. tu1 = mdec11
                                        lin22 = 1
                                   ENDIF
                                   li = li + 1
                                   IF li > 21
...
@ 00, 00 SAY 'ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป'
@ 01, 00 SAY 'บ C.I. :            Alumno:                                                  บ'
@ 02, 00 SAY 'บ Menciขn :                   Semestre    Plan de Cobro:       Lapso:        บ'
@ 03, 00 SAY 'วฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤถ'
@ 04, 00 SAY 'บTot.Deuda Vencida Mens.:               Fecha de Pago    :                   บ'
@ 05, 00 SAY 'บ   Monto Pagado/Abonado:               Tipo Pago (E/D/C):                   บ'
@ 06, 00 SAY 'บ  Saldo Pendiente Pagar:               Nro. Dep./Cheq   :                   บ'
@ 07, 00 SAY 'บ            % Descuento:               Banco :                              บ'
@ 08, 00 SAY 'บ  Monto Cancelar/Abonar:               Cobrador :                           บ'
@ 09, 00 SAY 'บ  Nro. ณ  Descripciขn de Conceptos           ณ Fecha Vencmto. ณ Monto Cuota บ' COLOR N/W 
@ 10, 00 SAY 'บ       ณ                                     ณ                ณ             บ'
@ 11, 00 SAY 'บ       ณ                                     ณ                ณ             บ'
@ 12, 00 SAY 'บ       ณ                                     ณ                ณ             บ'
@ 13, 00 SAY 'บ       ณ                                     ณ                ณ             บ'
...
@ 19, 00 SAY 'บ       ณ                                     ณ                ณ             บ'
@ 20, 00 SAY 'บ       ณ                                     ณ                ณ             บ'
@ 21, 00 SAY 'วฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤถ'
@ 22, 00 SAY 'บ                                                              ณ             บ'
SELECT 7
xlap = lapsomat
m.total_d = total_d - monto_p
ch1 = LEFT(xlap, 2)
ch2 = ALLTRIM(STR(YEAR(CTOD('01/01/' + SUBSTR(xlap, 5, 2)))))
IF lapsomat <> SPACE(6)
     SELECT 18
     ...
     SCATTER MEMO MEMVAR
     m.curso = cur
     m.saldo = m.total_deud - m.monto_paga
     IF m.saldo = 0
...
@ 04, 26 GET m.saldo PICTURE '99,999,999.99'
@ 05, 26 GET m.monto_p PICTURE '99,999,999.99'
@ 06, 26 GET m.saldo PICTURE '99,999,999.99'
CLEAR GETS
RELEASE datos
DIMENSION datos( 7, 4)
...
IF m.cur <> SPACE(5)
     FOR q = 1 TO 12
          lq = LTRIM(STR(q))
          mpagar=cuota&lq-pagado&lq
          if pagado&lq >= cuota&lq
               LOOP
          ENDIF
          mcuota = q
          SELECT 11
          LOCATE FOR lapso = xlap .AND. codigo =  m.plancobro  .AND. cuota = q
          xfec = fecha_ven
          xcon = concepto1
          SELECT 12
          IF  .NOT. SEEK(xcon)
...
DO WHILE .T.
     IF RLOCK()
          REPLACE monto_p WITH monto_p + m.montop
          REPLACE descto WITH m.descuento
          IF total_d = monto_p
             replace monto1 with 0.00
             replace monto2 with 0.00
             replace monto3 with 0.00             
             replace monto4 with 0.00             
          ENDIF        
          replace concepto5 with alltrim(concepto5)+" RECIBO: "+LTRIM(STR(mrecibo))+" "+"FECHA PAGO: "+DTOC(m.fechap)       
          UNLOCK
          EXIT
     ENDIF
ENDDO
     ...
     IF RLOCK()
          ...
          FOR ii = 1 TO 12
               ...
               if (mpagado+pagado&lk) >= m.cuota&lk
                    ...
                    REPLACE nrorecibo  WITH mrecibo
                    REPLACE fecha_ultp WITH m.fechap
               ELSE
                    repla pagado&lk with pagado&lk+mpagado
                    REPLACE monto_paga WITH monto_paga + mpagado
                    REPLACE nrorecibo  WITH mrecibo
...
@ 04, 00 SAY  ;
  'บTot.Deuda Vencida Adic.:               Fecha de Pago    :                   บ'
@ 05, 00 SAY  ;
...
IF lapsomat <> SPACE(6)
     SELECT 18
     use ad&xlap again alias nuevo order;
ced
     SEEK xcedula
     SCATTER MEMO MEMVAR
     m.saldo = m.total_deud -  ;
               m.monto_paga
     IF m.saldo = 0
          RELEASE WINDOW deuda
          RETURN .F.
...
ELSE
     ...
     ENDFOR
ENDIF
@ 22, 65 SAY m.saldo PICTURE '99,999,999.99'
m.fechap = DATE()
m.tipop = 'Efectivo'
m.nrode = SPACE(20)
m.banco = SPACE(2)
...
DO WHILE .T.
     IF FLOCK()
          APPEND BLANK
          REPLACE recibo WITH mrecibo, cedula WITH m.cedula
          REPLACE curso WITH m.curso,plancobro WITH  m.plancobro
          REPLACE seccion WITH  m.semestre, fecha_pago WITH m.fechap
          REPLACE nrocuota WITH 0,tipo_pago WITH m.tipop, nro_depo WITH m.nrode
          REPLACE cod_banco WITH m.banco, monto WITH m.montop, transa WITH 'AT'
          REPLACE cobrador WITH m.cobrador
          REPLACE lapso WITH lapmat
          UNLOCK
          EXIT
     ENDIF
