...
@ 04, 00 SAY  ;
  '³ Plan Estudio:         Semestre:          Turno:        Fecha:             ³'
@ 05, 00 SAY  ;
...
DO WHILE .T.
     ...
     DO CASE
          ...
          CASE m.curso = '31022'
               IF VAL(m.codigo) =  ;
                  01
                    m.semetre = 'PRIMER SEMESTRE DE CIENCIAS'
                    m.equivale = '1er. A¥O'
               ENDIF
               IF VAL(m.codigo) =  ;
                  02
                    m.semetre = 'SEGUNDO SEMESTRE DE CIENCIAS'
                    m.equivale = '1er. A¥O'
               ENDIF
               IF VAL(m.codigo) =  ;
                  03
                    m.semetre = 'TERCER SEMESTRE DE CIENCIAS'
                    m.equivale = '2do. A¥O'
               ENDIF
               IF VAL(m.codigo) =  ;
                  04
                    m.semetre = 'CUARTO SEMESTRE DE CIENCIAS'
                    m.equivale = '2do. A¥O'
               ENDIF
          CASE m.curso = '31023'
               IF VAL(m.codigo) =  ;
                  01
                    m.semetre = 'PRIMER SEMESTRE DE HUMANIDADES'
                    m.equivale = '1er. A¥O'
               ENDIF
               IF VAL(m.codigo) =  ;
                  02
                    m.semetre = 'SEGUNDO SEMESTRE DE HUMANIDADES'
                    m.equivale = '1er. A¥O'
               ENDIF
               IF VAL(m.codigo) =  ;
                  03
                    m.semetre = 'TERCER SEMESTRE DE HUMANIDADES'
                    m.equivale = '2do. A¥O'
               ENDIF
               IF VAL(m.codigo) =  ;
                  04
                    m.semetre = 'CUARTO SEMESTRE DE HUMANIDADES'
                    m.equivale = '2do. A¥O'
               ENDIF
     ENDCASE
     IF m.turno = 'MAT'
     ...
     @ 04, 00 SAY SPACE(((80 -  ;
       LEN(y4) - 6) / 2) + 3) +  ;
       y4
     centro1 = 'C O N S T A N C I A        D E        E S T U D I O'
     @ 11, 00 SAY SPACE(((80 -  ;
       LEN(centro1) - 6) / 2) +  ;
       3) + centro1
     @ 18, 05 SAY  ;
     ...
     @ 22, 05 SAY xnombre
     @ 24, 05 SAY  ;
       'de Identidad Nro.____________, cursa estudios en esta instituci¢n en el'
     @ 24, 23 SAY xcedula PICTURE  ;
       '!-9999999999'
     DO CASE
          CASE m.curso = '32012'
               @ 26, 05 SAY  ;
                 '________________________   (_______________    de   Educaci¢n  B sica),'
               @ 26, 06 SAY  ;
                 m.semetre
               @ 26, 34 SAY  ;
                 m.equivale
               ...
               @ 30, 05 SAY  ;
                 'el turno ____________ en  el A¤o Escolar : ' +  ;
                 ch1 + '-' + ch2 +  ;
                 '.'
               @ 30, 15 SAY  ;
                 m.turnito
          CASE m.curso = '31022'
               @ 26, 05 SAY  ;
                 '__________________________________ (_____________  de  Educaci¢n  Media'
               @ 26, 06 SAY  ;
                 m.semetre
               @ 26, 42 SAY  ;
                 m.equivale
               @ 28, 05 SAY  ;
                 'Diversificada)  del  plan  de  estudio  de  la Modalidad  Educaci¢n  de'
               @ 30, 05 SAY  ;
                 'Adultos, durante el turno ____________ en el A¤o Escolar:' +  ;
                 ch1 + '-' + ch2 +  ;
                 '.'
               @ 30, 32 SAY  ;
                 m.turnito
